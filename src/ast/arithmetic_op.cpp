...
CompiledValue ArithmeticOp::Compile(const letsjit::compilation::Context& ctx) const {
  const auto& left = dynamic_cast<const AbstractExpression&>(GetLeft()).Compile(ctx);
  const auto& right = dynamic_cast<const AbstractExpression&>(GetRight()).Compile(ctx);
